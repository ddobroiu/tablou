...
export async function generateStaticParams() {
  const slugs = getAllProductSlugsByCategory("fonduri-regio");
...
export async function generateMetadata({ params }: Props) {
  ...
  const { product, isFallback } = await resolveProductForRequestedSlug(String(raw), "fonduri-regio");
  if (!product) return {};
...
export default async function Page({ params }: Props) {
  ...
  const { product } = await resolveProductForRequestedSlug(String(joinedSlug), "fonduri-regio");

  if (!product) return notFound();

  const url = `${process.env.NEXT_PUBLIC_SITE_URL ?? ""}/fonduri-regio/${joinedSlug}`;
