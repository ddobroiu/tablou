...
export default function TrackOrderPage() {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            case "active": return { label: "În Producție", color: "text-emerald-600", bg: "bg-emerald-50", icon: Clock, step: 1 };
    ...
    return (
        <main className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center pt-28">

            {/* LOGO */}
            <div className="mb-8">
                <Link href="/">
                    <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600">Tablou</span>
                </Link>
            </div>

            {/* HEADER */}
            <div className="text-center mb-10 max-w-xl">
                <div className="inline-flex items-center justify-center p-3 bg-white rounded-2xl shadow-sm mb-4 text-emerald-600">
                    <Search size={32} />
                </div>
                    ...
                    <form onSubmit={handleTrack} className="space-y-5">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-8 pr-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none font-medium text-slate-900 placeholder:text-slate-500 transition-all"
                                    placeholder="Ex: 1024"
                                    required
                                />
                            </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none font-medium text-slate-900 placeholder:text-slate-500 transition-all"
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-emerald-200 transition-all hover:scale-[1.02] disabled:opacity-70 disabled:hover:scale-100 flex items-center justify-center gap-2"
                        >
                            {loading ? (
                        ...
                        <div className="p-6 sm:p-8">
                            ...
                            <div className="mb-10">
                                ...
                                <div className="relative">
                                    ...
                                    <div
                                        className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-emerald-600 rounded-full -z-10 transition-all duration-1000"
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        {/* Step 2: Producție */}
                                        <div className="flex flex-col items-center gap-2">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-colors ${getStatusInfo(order.status).step >= 1 ? 'bg-emerald-600 text-white shadow-emerald-200' : 'bg-white border-2 border-slate-200 text-slate-300'}`}>
                                                <Package size={16} />
                                            </div>
                                ...
                                {/* AWB Info */}
                                {order.awb && (
                                    <div className="mt-8 p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 bg-white rounded-lg text-emerald-600 shadow-sm"><Truck size={20} /></div>
                                            <div>
                                ...
                                <ul className="divide-y divide-slate-100">
                                    {order.items.map((item: any, i: number) => (
                                        <li key={i} className="py-3 flex justify-between items-center text-sm">
                                            ...
                                            <span className="text-slate-500 font-semibold">x{item.quantity || item.qty}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
