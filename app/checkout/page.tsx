...
export default function CheckoutPage() {
  ...
  function fromFormBilling(prev: BillingInfo, formBill: any): BillingInfo {
    ...
    return {
      ...
      country: formBill?.country ?? prev.country,
    };
  }
  ...
  useEffect(() => {
    ...
    // 1. Populăm datele de bază din sesiune DOAR dacă câmpurile sunt goale
    setAddress((prev) => {
    ...
    // 2. Încercăm să preluăm adresele salvate din baza de date
    const fetchAddresses = async () => {
      try {
        ...
        if (savedAddresses.length > 0) {
          // Căutăm adresa implicită sau o folosim pe prima
          const defaultAddr = savedAddresses.find((a: any) => a.isDefault) || savedAddresses[0];

          setAddress((prev) => {
            // Suprascriem DOAR dacă adresa fizică (strada) este goală
            if (prev.street) return prev;

            // Mapăm numele - dacă în DB avem un singur câmp 'nume', încercăm să-l desfacem
            const fullName = defaultAddr.nume || "";
            let fName = fullName;
            let lName = "";
            ...
            return mergeAddress(prev, {
              ...
              phone: defaultAddr.telefon || prev.phone,
              county: defaultAddr.judet,
              city: defaultAddr.localitate,
              street: defaultAddr.strada_nr,
              postalCode: defaultAddr.postCode || "",
            });
          });
        }
      } catch (err) {
  ...
  async function onPlaceOrder() {
    ...
    const payload = {
      ...
      discountAmount,
      subtotal,
      shippingCost: shippingCostForPayload,
      totalWithShipping,
    };
        ...
        <div className="grid lg:grid-cols-[minmax(0,2fr)_minmax(0,1.25fr)] gap-6 lg:gap-8">
            ...
            <header className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-6 shadow-sm">
              <div>
                <h1 className="text-xl sm:text-2xl font-black text-black dark:text-white flex items-center gap-2">
                  Finalizare Comandă
                  <span className="inline-flex items-center justify-center rounded-full bg-emerald-600 text-white text-xs font-bold px-3 py-1 shadow-sm">
          ...
          <aside className="lg:col-span-1 lg:sticky lg:top-6 space-y-4 lg:space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
              <div className="bg-gradient-to-r from-teal-600 to-emerald-600 px-4 sm:px-5 py-3">
                <h2 className="text-base sm:text-lg font-semibold text-white flex items-center gap-3">
                  <span className="w-7 h-7 rounded-lg bg-white/20 text-white flex items-center justify-center text-sm font-black shadow-sm ring-1 ring-white/30">3</span>
                  Metodă de plată
                </h2>
              </div>
                ...
                <div className="grid gap-3">
                  <label
                    className={`flex items-center gap-3 rounded-xl border-2 p-3 cursor-pointer transition ${paymentMethod === "card"
                      ? "border-emerald-600 bg-emerald-50 dark:bg-emerald-950 ring-2 ring-emerald-200 dark:ring-emerald-800"
                      : "border-slate-300 dark:border-slate-700 hover:border-emerald-400 hover:bg-slate-50/60 dark:hover:bg-slate-900/60"
                      }`}
                  >
                    <input
                      type="radio"
                      ...
                      className="hidden"
                    />
                    <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "card" ? "bg-emerald-600 border-emerald-600" : "bg-white dark:bg-slate-900 border-emerald-400"}`}>
                      <CreditCard className={`h-5 w-5 ${paymentMethod === "card" ? "text-white" : "text-emerald-600"}`} />
                    </div>
                  ...
                  >
                    <input
                      ...
                      className="hidden"
                    />
                    <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "bank_transfer" ? "bg-emerald-600 border-emerald-600" : "bg-white dark:bg-slate-900 border-emerald-400"}`}>
                      <Banknote className={`h-5 w-5 ${paymentMethod === "bank_transfer" ? "text-white" : "text-emerald-600"}`} />
                    </div>
                    <div className="flex-1">
                      <p className="font-semibold text-slate-900 dark:text-white">
                  ...
                  {paymentMethod === "bank_transfer" && (
                    <div className="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/80 dark:bg-emerald-950/40 p-4 text-sm space-y-2">
                      <p className="font-semibold text-slate-900 dark:text-white text-xs uppercase tracking-wide">
                        Date pentru plată
                      </p>
                      <div className="flex justify-between gap-3">
                        <span className="text-slate-500 dark:text-slate-400 shrink-0">Beneficiar</span>
                        <span className="text-slate-900 dark:text-white font-semibold text-right">{BANK_TRANSFER_BENEFICIARY}</span>
                      </div>
                      <div className="flex justify-between gap-3">
                        <span className="text-slate-500 dark:text-slate-400 shrink-0">IBAN (RON)</span>
                        <span className="text-emerald-700 dark:text-emerald-400 font-bold select-all text-right">{BANK_TRANSFER_IBAN}</span>
                      </div>
                      <div className="flex justify-between gap-3">
                        <span className="text-slate-500 dark:text-slate-400 shrink-0">Banca</span>
                        <span className="text-slate-900 dark:text-white font-semibold text-right">{BANK_TRANSFER_BANK_NAME}</span>
                      </div>
                    </div>
                  )}
                  ...
                  {!isRambursDisabled && (
                    ...
                    >
                      <input
                        ...
                        className="hidden"
                      />
                      <div className={`flex-shrink-0 flex items-center justify-center h-9 w-9 rounded-full border-2 ${paymentMethod === "cash_on_delivery" ? "bg-emerald-600 border-emerald-600" : "bg-white dark:bg-slate-900 border-emerald-400"}`}>
                        <Truck className={`h-5 w-5 ${paymentMethod === "cash_on_delivery" ? "text-white" : "text-emerald-600"}`} />
                      </div>
                      <div className="flex-1">
                        <p className="font-semibold text-slate-900 dark:text-white">
                          Ramburs la curier
                ...
                <div className="border-t border-slate-200 dark:border-slate-800 pt-3 mt-3 space-y-2 text-sm">
                  ...
                  {!hasFreeShipping && (
                    <p className="text-xs text-slate-700 dark:text-slate-300 flex items-start gap-1 mt-1">
                      <AlertCircle className="h-3.5 w-3.5 mt-0.5 text-emerald-500" />
                      <span>
                        Mai adaugă{" "}
                        <span className="font-semibold text-emerald-600 dark:text-emerald-400">
  ...
  if (placing) {
    return (
      <button
        type="button"
        disabled
        className="inline-flex w-full items-center justify-center rounded-xl border border-transparent bg-emerald-500 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition"
      >
        <svg
...
function CartItems({
  ...
  const labelForKey: Record<string, string> = {
    ...
    // Master Configurator exact keys mapping
    Thickness_mm: "Grosime (mm)",
    ...
    print_type: "Tip Print",
    PaperWeightKey: "Greutate/Grosime (g/mp)",
    paperWeightKey: "Greutate/Grosime (g/mp)",
    FrameType: "Ramă/Sistem",
    frameType: "Ramă/Sistem",
    Edge_type: "Tip Margine",
    edge_type: "Tip Margine",
    Want_hem_and_grommets: "Tiv și capse",
    Artwork_distribution: "Distribuție Grafică Față-Verso",
    artwork_distribution: "Distribuție Grafică",
    Sides_count: "Număr Fețe",
    sides_count: "Număr Fețe",
    Material_type: "Tip Material",
    material_type: "Tip Material"
  };
