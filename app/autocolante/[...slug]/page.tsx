...
export async function generateMetadata({ params }: Props) {
  try {
    ...
    console.error("Error generating metadata for autocolante:", error);
    return {
      title: "Produs",
...
export default async function Page({ params, searchParams }: { params: Promise<{ slug?: string[] }>, searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  try {
    ...
    // Determinăm titlul și descrierea: dacă e produs multi-variant, căutăm varianta Autocolant
    ...
    if (product.metadata?.isMultiVariant && product.metadata?.variants) {
      const autocolantVariant = (product.metadata.variants as any[]).find((v: any) => v.type === 'autocolant');
      if (autocolantVariant) {
        finalTitle = autocolantVariant.title;
        finalDescription = autocolantVariant.description;
      }
    }
    ...
    console.error("Error rendering page for autocolante:", error);
    return notFound();
  }
}