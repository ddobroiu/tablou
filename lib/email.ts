...
export async function sendWelcomeEmail(to: string, name: string, source?: string) {
  try {
    ...
    console.log(`[EMAIL] Attempting to send welcome email to ${to} from ${process.env.EMAIL_FROM || config.email}`);
...
export async function sendOrderConfirmationEmail(order: any, customContent?: string) {
  try {
    ...
    console.log(`[EMAIL] Attempting to send order confirmation for #${orderNo} to ${(order.shippingAddress as any)?.email || order.userEmail || order.email} from ${process.env.EMAIL_FROM || config.email}`);
    const result = await sendEmail({
      from: `${config.name} <${process.env.EMAIL_FROM || config.email}>`,
    ...
    if (!result.error && (result as any).data) {
      console.log(`[EMAIL] Order confirmation sent successfully. ID: ${(result as any).data?.id}`);
    }
  } catch (error) {
...
export async function sendNewOrderAdminEmail(order: any, customContent?: string) {
  try {
    ...
    if (customContent) {
      ...
      content = `
        <div style="background:#f1f5f9; padding: 16px; border-radius: 10px; margin-bottom: 18px; border: 1px solid #e2e8f0;">
          <div style="display:flex; justify-content:space-between; gap: 12px; align-items:flex-start;">
            <div style="flex: 1;">
              <div style="font-size:12px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:0.12em;">Client</div>
              <div style="font-size:14px; color:#0f172a; font-weight:800; margin-top:4px;">${escapeHtml(ship.name || ship.nume_prenume || '')}</div>
              <div style="font-size:13px; color:#475569; margin-top:4px;">
                ${ship.email ? `<div><strong>Email:</strong> ${escapeHtml(ship.email)}</div>` : ``}
                ${(ship.phone || ship.telefon) ? `<div><strong>Telefon:</strong> ${escapeHtml(ship.phone || ship.telefon)}</div>` : ``}
              </div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:12px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:0.12em;">Total</div>
              <div style="font-size:18px; color:#0f172a; font-weight:900; margin-top:4px;">${total.toFixed(2)} RON</div>
            </div>
          </div>
        </div>
        ${customContent}
    ...
    const html = getPremiumHtmlTemplate({
      ...
      buttonUrl: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://www.prynt.ro'}/admin/orders/${order.id}`,
      brandConfig: config
    });

    console.log(`[EMAIL] Attempting to send admin notification for #${orderNo} to ${adminEmail} from ${process.env.EMAIL_FROM || siteConfig.email}`);
    const result = await sendEmail({
    ...
    if (!result.error && (result as any).data) {
      console.log(`[EMAIL] Admin notification sent successfully. ID: ${(result as any).data?.id}`);
    }
  } catch (error) {
...
export async function sendPasswordResetEmail(to: string, token: string, source?: string) {
  try {
    ...
    console.log(`[EMAIL] Attempting to send password reset to ${to} from ${process.env.EMAIL_FROM || config.email}`);
    const result = await sendEmail({
      from: `${config.name} <${process.env.EMAIL_FROM || config.email}>`,
      to,
    ...
    if (!result.error && (result as any).data) {
      console.log(`[EMAIL] Password reset email sent successfully. ID: ${(result as any).data?.id}`);
    }
  } catch (error) {
...
export async function sendContactFormEmail({ name, email, phone, message, subject: userSubject, source }: { name: string, email: string, phone?: string, message: string, subject?: string, source?: string }) {
  try {
    ...
    console.log(`[EMAIL] Attempting to send contact form email from ${email} to ${adminEmail} using ${process.env.EMAIL_FROM || config.email}`);
    const result = await sendEmail({
      from: `${config.name} Form <${process.env.EMAIL_FROM || config.email}>`,
      to: adminEmail,
      replyTo: email,
    ...
    if (!result.error && (result as any).data) {
      console.log(`[EMAIL] Contact form email sent successfully. ID: ${(result as any).data?.id}`);
    }
  } catch (error) {
...
export async function sendEmail({ from, to, subject, html, text, attachments, bcc, replyTo }: SendEmailParams) {
  try {
    const resend = getResend();
    if (!resend) {
