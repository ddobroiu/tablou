...
<body>
  <div class="container">
    ...
    <div class="footer">
      © ${new Date().getFullYear()} Tablou.net. Toate drepturile rezervate.<br/>
...
export async function sendPasswordResetEmail(to: string, token: string) {
  ...
  const html = getHtmlTemplate({
    ...
    message: "Am primit o cerere de resetare a parolei pentru contul tău Tablou.net. Apasă pe butonul de mai jos pentru a seta o parolă nouă.",
...
export async function sendOrderConfirmationEmail(order: any) {
  ...
  const bankDetailsHtml = isOP ? `
    <div style="background-color: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 16px; margin: 24px 0;">
      ...
      <p style="font-size: 13px; margin-top: 12px; color: #0369a1; border-top: 1px solid #bae6fd; padding-top: 8px;">
        <strong>Important:</strong> Pentru o procesare rapidă, te rugăm să trimiți dovada plății (OP-ul) pe email la adresa <a href="mailto:contact@tablou.net" style="color: #0369a1; text-decoration: underline;">contact@tablou.net</a>.
      </p>
    </div>
  ` : '';
  ...
  const html = getHtmlTemplate({
    ...
    buttonUrl: viewUrl,
    footerText: "Mulțumim că ai ales Tablou.net!"
  });
...
export async function sendNewOrderAdminEmail(order: any) {
  ...
  const message = `
    ...
    • Total: <strong>${order.totalAmount} ${order.currency || 'RON'}</strong><br/>
    • Metodă plată: ${order.paymentMethod || 'N/A'}<br/>
