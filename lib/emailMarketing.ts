...
export const EMAIL_CATEGORIES = {
  outdoor: {
    ...
    products: ['banner', 'banner-verso', 'mesh'],
...
export function getSmartRecommendations(userHistory: string[], currentInterest?: string): typeof MAIN_CONFIGURATORS[number][] {
  ...
  const crossSellMap: Record<string, string[]> = {
    'banner': ['autocolante', 'afise', 'pvc-forex', 'mesh'],
    'banner-verso': ['banner', 'mesh', 'plexiglass', 'alucobond'],
    'mesh': ['banner', 'banner-verso', 'autocolante'],
...
export function generateNewsletterCampaign(theme: 'weekly' | 'promotional' | 'educational') {
  ...
  switch (theme) {
    case 'weekly':
      return {
        subject: 'Săptămâna aceasta la Tablou: Noutăți și Oferte',
...
export async function sendConfiguratorWelcomeEmail(subscription: NewsletterSubscription) {
  ...
  const html = getHtmlTemplate({
    ...
    footerText: "Mulțumim că te-ai alăturat comunității Tablou!"
  });
  ...
  await sendEmail({
    from: 'Tablou <contact@Tablou.net>',
...
export async function sendPostPurchaseFollowUp(email: string, name: string, orderId: string, orderNo: number) {
  ...
  const html = getHtmlTemplate({
    title: `Bună ${name}, cum ți s-au părut produsele Tablou?`,
    ...
    footerText: "Echipa Tablou vă mulțumește!"
  });
  ...
  try {
    await sendEmail({
      from: 'Tablou <contact@Tablou.net>',
      to: email,
      subject: `🎁 Avem un cadou pentru tine (Comanda ta #${orderNo} la Tablou)`,
      html: enhancedHtml,
    });
    return true;
  } catch (error) {
...
export async function sendAbandonedCartEmail({ email, configuratorId, cartData, emailType, discountPercent = 0 }: {
  ...
  try {
    switch (emailType) {
      case 'gentle':
        subject = `🎨 Te așteaptă ceva special la Tablou!`;
