...
export default function AdminAwbControl({ orderId, currentAwb }: { orderId: string; currentAwb?: string | null }) {
  ...
  const generateAwb = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Eroare necunoscută la generare AWB');
      }
      ...
      alert("Eroare generare AWB DPD: " + (error.message || "Verifică datele de livrare (localitate, cod poștal etc.)"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateAwb = async () => {
    ...
    try {
      ...
      if (data?.url) {
        ...
        alert("Nu s-a putut genera link-ul de validare.");
      }
    } catch (error) {
      alert("Eroare validare AWB.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <a href={`https://tracking.dpd.ro/?shipmentNumber=${awb}&language=ro`} target="_blank" className="flex items-center gap-1 text-[9px] text-emerald-300 hover:text-white hover:underline bg-emerald-500/10 py-1 px-2 rounded transition-colors">
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" variant="outline" onClick={generateAwb} disabled={loading} className="h-8 text-[10px] bg-white text-black hover:bg-zinc-100 border-gray-200 flex-1 font-semibold">
          {loading ? <Loader2 size={12} className="animate-spin" /> : <><Plus size={12} className="mr-1" /> Emite Direct</>}
        </Button>
      </div>
