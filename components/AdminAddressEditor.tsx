  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <div className="group cursor-pointer p-2 rounded-lg hover:bg-white dark:bg-slate-900/5 transition-colors border border-transparent hover:border-white/5">
            <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-1.5 font-semibold text-white text-xs">
                    <User size={12} className="text-zinc-500" />
                    {address.nume_prenume || "Nume lipsă"}
                </div>
                <Edit2 size={10} className="opacity-0 group-hover:opacity-100 text-emerald-400 transition-opacity" />
            </div>
            ...
            <div className="flex items-center gap-1.5 mt-1.5 text-[11px] text-emerald-300 font-mono pl-0.5">
                <Phone size={10} />
                {address.telefon}
            </div>
        </div>
      ...
      <DialogContent className="bg-[#09090b] border-white/10 text-white">
        ...
        <div className="flex justify-end gap-2">
            ...
            <Button onClick={saveAddress} className="bg-emerald-600 text-white hover:bg-emerald-500">Salvează</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
