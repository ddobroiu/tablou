...
export default function HeaderFreeShipping() {
  ...
  // Afișăm mesaj informativ chiar dacă nu e nimic în coș
  if (cartTotal === 0) {
    return (
      <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-white/50 border border-slate-200 rounded-full">
        <Truck size={14} className="text-slate-500" />
        <span className="text-xs font-medium text-slate-600">
          Livrare Gratuită &gt; <b>{formatMoneyDisplay(FREE_SHIPPING_THRESHOLD)}</b>
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="hidden lg:flex items-center gap-2 px-2 lg:px-3 py-1.5 bg-white dark:bg-emerald-950/30 border-2 border-slate-200 dark:border-emerald-800 rounded-lg shadow-sm">
      {remainingForFreeShipping > 0 ? (
        <>
          ...
          <div className="relative h-1.5 w-12 lg:w-16 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden shrink-0">
            <div
              className="absolute top-0 left-0 h-full bg-gradient-to-r from-emerald-500 to-purple-500 transition-all duration-700 rounded-full"
