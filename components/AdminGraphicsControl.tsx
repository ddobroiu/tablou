    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="bg-[#09090b] border-white/10 text-white max-w-3xl">
        ...
        <div className="space-y-4 py-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
          {items.map((item, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900/5 p-4 rounded-xl border border-white/5 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                
                {/* Detalii Produs */}
                <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-sm text-white truncate">{item.productName || "Produs Custom"}</h4>
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-zinc-400 bg-white dark:bg-slate-900/5 px-2 py-0.5 rounded">
