...
const ProductTabs = ({ productSlug }: { productSlug: string }) => {
  ...
  const faq: QA[] = [
    { question: "Ce este Akyplac (Polipropilena)?", answer: "Akyplac este un material ușor, cu structură celulară (similar cu cartonul ondulat, dar din plastic). Este impermeabil, durabil și economic." },
    { question: "Pentru ce se folosește?", answer: "Este ideal pentru panouri imobiliare (DE VÂNZARE / DE ÎNCHIRIAT), semnalistică temporară de șantier, afișaje în magazine sau ambalaje reutilizabile." },
    { question: "Rezistă la exterior?", answer: "Da, polipropilena este rezistentă la apă și intemperii, fiind o soluție excelentă pentru afișaj outdoor pe termen scurt și mediu." },
  ];
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800">
      ...
      <div className="p-6 lg:p-8">
        {activeTab === 'descriere' && (
          <>
            <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-4">Polipropilenă Celulară (Akyplac)</h2>
            <p className="text-gray-700 dark:text-gray-300 mb-8 leading-relaxed">Plăci ușoare și rigide din plastic celular, perfecte pentru panouri temporare, imobiliare și de șantier. O alternativă rezistentă la apă pentru cartonul clasic.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="flex flex-col items-center text-center p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-purple-50 border border-emerald-100">
                ...
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-2">Polipropilena Rezistentă</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">Material plastic rigid, nu se rupe, foarte ușor</p>
              </div>
              ...
              <div className="flex flex-col items-center text-center p-4 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100">
                ...
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-2">Ideal Exterior</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">Rezistă perfect la ploaie și umiditate, durabil</p>
              </div>
              ...
              <div className="flex flex-col items-center text-center p-4 rounded-xl bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-100">
                ...
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-2">Raport Calitate/Preț</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">Alternativă economică la PVC Forex sau alucobond</p>
              </div>
              ...
              <div className="flex flex-col items-center text-center p-4 rounded-xl bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100">
                ...
                <h3 className="text-base font-bold text-gray-900 dark:text-white mb-2">Print UV Direct</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400">Culori rezistente, suprafață albă strălucitoare</p>
              </div>
            </div>
          </>
        )}
...
export default function ConfiguratorPolipropilena({ productSlug, initialWidth: initW, initialHeight: initH, productImage }: Props) {
  ...
  function handleAddToCart() {
    ...
    const uniqueId = `${productSlug ?? 'polipropilena'}-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
    const title = `Placă Polipropilenă ${input.thickness_mm}mm - ${input.width_cm}x${input.height_cm} cm`;

    addItem({
      id: uniqueId,
      productId: productSlug ?? "polipropilena",
      slug: productSlug ?? "polipropilena",
      title,
      ...
      metadata: {
        ...
        "Gramaj": `${POLIPROPILENA_CONSTANTS.GRAMAJ[input.thickness_mm]} g/mp`,
  ...
  const summaryStep2 = `Grosime: ${input.thickness_mm}mm (${POLIPROPILENA_CONSTANTS.GRAMAJ[input.thickness_mm]} g/mp)`;
  ...
  return (
    <div className="w-full">
      <div className="container mx-auto px-4 py-10 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="lg:sticky top-24 h-max space-y-8">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 overflow-hidden">
              <div className="aspect-square"><img src={activeImage} alt="Polipropilena" className="h-full w-full object-cover" /></div>
          ...
          <div>
            <header className="mb-6">
              <div className="flex justify-between items-center gap-4 mb-3"><h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Configurator Polipropilenă</h1></div>
              <div className="flex justify-between items-center"><p className="text-gray-600 dark:text-gray-400">Personalizează placa în 3 pași simpli.</p><button type="button" onClick={() => setDetailsOpen(true)} className="btn-outline inline-flex items-center text-sm px-3 py-1.5"><Info size={16} /><span className="ml-2">Detalii</span></button></div>
            </header>
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 px-4">
              <AccordionStep stepNumber={1} title="Dimensiuni & Cantitate" summary={summaryStep1} isOpen={activeStep === 1} onClick={() => setActiveStep(1)}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <p className="text-xs text-gray-400 md:col-span-2">Maxim: {POLIPROPILENA_CONSTANTS.LIMITS.MAX_WIDTH}x{POLIPROPILENA_CONSTANTS.LIMITS.MAX_HEIGHT} cm</p>
                </div>
              </AccordionStep>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      {POLIPROPILENA_CONSTANTS.AVAILABLE_THICKNESS.map(t => (
                        <OptionButton key={t} active={input.thickness_mm === t} onClick={() => updateInput("thickness_mm", t)} title={`${t} mm`} subtitle={`${POLIPROPILENA_CONSTANTS.GRAMAJ[t]} g/mp`} />
                      ))}
                    </div>
                  </div>
                </div>
                    ...
                    <>
                      ...
                      {input.designOption === 'pro' && (
                        <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">
                          ...
                          <p>Cost: <strong>{formatMoneyDisplay(POLIPROPILENA_CONSTANTS.PRO_DESIGN_FEE)}</strong>. Un designer te va contacta.</p>
                        </div>
                      )}
                    </>
                  )}
            ...
            <div className="mt-4 lg:mt-6 bg-gradient-to-br from-slate-50 to-gray-100 dark:from-slate-800 dark:to-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a
                  href="https://wa.me/40750473111?text=Ma%20intereseaza%20configuratorul%20polipropilena"
                  target="_blank"
        ...
        <div className="mt-8 lg:mt-12"><ProductTabs productSlug={productSlug || 'polipropilena'} /></div>
      ...
      {detailsOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setDetailsOpen(false)}>
          <div className="relative z-10 w-full max-w-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 p-8" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Detalii Polipropilenă</h3>
            <div className="prose dark:prose-invert prose-sm prose-slate max-w-none">
              <h4>Material</h4>
              <p>Polipropilenă celulară (cunoscută și ca Akyplac sau Coroplast), structură tip fagure, culoare alb mat.</p>
              <h4>Grosimi</h4>
              <ul>
                <li><strong>3mm:</strong> Economic, ușor, pentru panouri mici/medii.</li>
                <li><strong>5mm:</strong> Rigiditate sporită, recomandat pentru panouri mai mari.</li>
              </ul>
            </div>
          </div>
        </div>
