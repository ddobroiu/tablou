...
export default function ProductCarousel({
    title = "Te-ar putea interesa și...",
    currentSlug,
    limit = 8,
    products: externalProducts,
    linkBase = 'canvas-product'
}: ProductCarouselProps) {
    const products = useMemo(() => {
        const source = externalProducts || canvasProducts;
        let filtered = source.filter(p => p.slug !== currentSlug);
        return filtered.slice(0, limit);
    }, [currentSlug, limit, externalProducts]);
        ...
        <section className="py-16 border-t border-gray-100 bg-white">
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <Link href="/configurator/canvas" className="text-emerald-600 font-bold text-sm hover:underline flex items-center gap-1">
                            ...
                            >
                                <Link href={`/${linkBase}/${product.slug}`} className="block h-full flex flex-col">
                                    <div className="relative aspect-[4/5] overflow-hidden bg-gray-100">
                                        ...
                                        {product.price && (
                                            <div className="absolute bottom-3 right-3">
                                                <span className="bg-white/90 backdrop-blur-sm text-slate-900 text-[10px] font-black px-2 py-1 rounded-md shadow-sm">
                                                    DE LA 79 LEI
                                                </span>
                                            </div>
                                        )}
                                    ...
                                    <div className="p-4 flex flex-col flex-grow">
                                        <h3 className="font-bold text-slate-900 line-clamp-1 group-hover:text-emerald-600 transition-colors mb-1">
                                            {product.title}
                                        </h3>
