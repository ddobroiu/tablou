...
export default function NewsletterSignup({ source = 'footer', configuratorId, compact = false }: Props) {
  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-purple-50 border border-emerald-200 rounded-2xl p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className={`text-xs px-3 py-2 rounded-lg border transition-colors ${
                    category.products.every(id => interests.includes(id))
                      ...
                      : 'bg-white dark:bg-slate-900 border-gray-300 text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  {category.name}
                </button>
        ...
        <div className="flex items-center justify-between">
          <button
            type="submit"
            disabled={status === 'loading' || !email}
            className="px-6 py-2 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 disabled:opacity-50 flex items-center gap-2"
          >
            {status === 'loading' && (
              <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
            )}
