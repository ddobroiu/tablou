...
// „Continua cu Google”: apare doar daca Google e configurat pe site (GOOGLE_CLIENT_ID/SECRET)
export default function GoogleSignInButton({ callbackUrl = "/account" }: { callbackUrl?: string }) {
    const [enabled, setEnabled] = useState(false);
    const [loading, setLoading] = useState(false);

    useEffect(() => {
        getProviders().then((p) => setEnabled(Boolean(p?.google))).catch(() => setEnabled(false));
    }, []);
    ...
    return (
        <div className="space-y-4">
            ...
            >
                ...
                {loading ? "Se deschide Google…" : "Continuă cu Google"}
            </button>
            <div className="flex items-center gap-3 text-xs text-slate-400">
                <span className="h-px flex-1 bg-slate-200" /> sau cu email <span className="h-px flex-1 bg-slate-200" />
            </div>
        </div>
    );
}
