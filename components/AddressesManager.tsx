        ...
        <div className="space-y-8">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
                <h2 className="text-xl font-bold mb-6 text-slate-900 dark:text-white flex items-center gap-2">
                    <span className="w-8 h-8 rounded-lg bg-emerald-600/10 text-emerald-600 flex items-center justify-center text-sm font-black">+</span>
                ...
                <div className="flex w-full overflow-hidden border border-slate-200 dark:border-slate-800 rounded-xl mb-6 p-1 bg-slate-50 dark:bg-slate-950 max-w-sm">
                    <button type="button" onClick={() => onForm("type", "shipping")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition-all duration-200 ${form.type === "shipping" ? "bg-white dark:bg-slate-800 text-emerald-600 shadow-sm" : "text-slate-500"}`}>Livrare</button>
                    <button type="button" onClick={() => onForm("type", "billing")} className={`flex-1 rounded-lg px-4 py-2 text-sm font-bold transition-all duration-200 ${form.type === "billing" ? "bg-white dark:bg-slate-800 text-emerald-600 shadow-sm" : "text-slate-500"}`}>Facturare</button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="md:col-span-3">
                        <Field id="form.strada_nr" label="Adresă (Stradă, număr, bloc, sc., etaj, ap.)">
                            <input className={inputCls()} placeholder="Adresa completă..." value={form.strada_nr} onChange={(e) => onForm("strada_nr", e.target.value)} />
                        </Field>
                    </div>
                    ...
                    <div className="md:col-span-4 flex items-center justify-between mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                            <input type="checkbox" checked={!!form.isDefault} onChange={(e) => onForm("isDefault", e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                            <span className="text-slate-600 dark:text-slate-400 font-medium">Setează ca implicită</span>
                        </label>
                        <button onClick={saveNew} disabled={saving} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-emerald-600/20 disabled:opacity-50">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <span className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-md border ${a.type === 'billing' ? 'bg-orange-500/10 text-orange-600 border-orange-500/20' : 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20'}`}>
                                        ...
                                        <div className="flex items-center gap-1.5 self-start">
                                            {a.type === 'shipping' && !a.isDefault && <button className="px-3 py-1.5 text-xs font-black uppercase text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-lg border border-emerald-600/20 transition-all" onClick={() => update(a.id!, { isDefault: true })}>Setează Implicit</button>}
...
function EditRow({ a, onCancel, onSave }: { a: Address; onCancel: () => void; onSave: (patch: Partial<Address>) => void }) {
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/30 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="md:col-span-2">
            ...
            <div className="md:col-span-3">
                <Field id="edit.strada_nr" label="Adresă">
            ...
            <div className="md:col-span-4 flex justify-end gap-2 mt-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button className="bg-emerald-600 text-white px-6 py-2 rounded-xl font-bold shadow-lg" onClick={submit} disabled={saving}>{saving ? '...' : 'Salvează'}</button>
            </div>
        </div>
    );
}
