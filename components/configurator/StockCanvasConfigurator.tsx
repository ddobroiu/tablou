...
export default function StockCanvasConfigurator({ productSlug, renderOnlyConfigurator }: Props) {
    ...
    const product = useMemo(() => {
        const found = canvasProducts.find(p => p.slug === productSlug);
        if (found) return found;

        // Fallback to landing info for localities/SEO pages
        const landingInfo = getLandingInfo("canvas", productSlug);
        if (landingInfo) {
            return {
                id: landingInfo.key,
                ...
                price: "89",
                category: "Canvas SEO",
                tags: ["seo", "canvas", landingInfo.key]
            } as any;
        }
        return undefined;
    }, [productSlug]);
    ...
    if (!product) {
        return (
            <div className="container mx-auto px-4 py-20 text-center">
                ...
                <Link href="/configurator/canvas" className="text-emerald-600 hover:underline mt-4 inline-block">
                    Înapoi la colecție
                </Link>
            </div>
        );
    ...
    const availableDimensions = useMemo(() => {
        ...
        if (product.dimensions && product.dimensions.length > 0) {
            ...
            if (d.includes('x')) {
                const [w, h] = d.split('x').map((s: string) => parseInt(s.replace(/\D/g, '')));
    ...
    const shareUrl = useMemo(() => {
        ...
        const url = new URL(`${window.location.origin}/canvas-product/${productSlug}`);
    ...
    // Select Model - Using New Stock Specific Variants
    let modelSrc = "/products/canvas/canvas_landscape_stoc.glb";
    let baseW = 60;
    let baseH = 40;
    ...
    if (Math.abs(displayW - displayH) < 1) {
        ...
        modelSrc = "/products/canvas/canvas_landscape_stoc.glb";
        baseW = 60;
        baseH = 40;
    }
    ...
    return (
        <div className={`bg-white w-full max-w-full overflow-x-hidden ${renderOnlyConfigurator ? '' : 'min-h-screen pb-20'}`}>
            ...
            <div className={`w-full max-w-full box-border ${renderOnlyConfigurator ? '' : 'container mx-auto px-4 py-4'}`}>
                {!renderOnlyConfigurator && (
                    <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
                        <Link href="/" className="hover:text-emerald-600">Home</Link>
                        <ChevronRight size={14} />
                        <Link href="/configurator/canvas" className="hover:text-emerald-600">Shop / Canvas</Link>
                        <ChevronRight size={14} />
                        <span className="text-slate-900 dark:text-white font-medium truncate max-w-[200px]">{product.title}</span>
                    </nav>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div className="lg:sticky lg:top-24 h-max space-y-4">
                        <div className="pb-4 border-b border-gray-100">
                            <div className="flex border-b border-gray-100 bg-slate-50 dark:bg-slate-800/50">
                                <button
                                    onClick={() => setViewMode('gallery')}
                                    className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${viewMode === 'gallery' ? 'text-emerald-600 bg-white border-b-2 border-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}
                                >
                                    <ImageIcon size={18} /> Foto
                                </button>
                                <button
                                    onClick={() => setViewMode('3d')}
                                    className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors ${viewMode === '3d' ? 'text-emerald-600 bg-white border-b-2 border-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:text-gray-300'}`}
                                >
                                    ...
                                    <div className="w-full h-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center relative group/viewer">
                                        ...
                                        >
                                            <button
                                                ...
                                                className={`absolute bottom-4 left-4 font-bold py-2 px-4 rounded-full shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] flex items-center gap-2 transition-transform active:scale-95 z-20 ${!isTextureLoaded ? 'bg-gray-400 cursor-not-allowed opacity-70 text-gray-100' : 'bg-emerald-600 hover:bg-emerald-700 text-white'}`}
                                            >
                        ...
                        {!renderOnlyConfigurator && (
                            <header className="mb-6">
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-3">
                                    <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white leading-tight break-words">
                                        {product.title}
                                    </h1>
                                    <span className="inline-flex w-max items-center px-4 py-1.5 rounded-full bg-red-600 text-white font-bold text-[10px] sm:text-xs animate-pulse shadow-md whitespace-nowrap">
                                        🔥 -20% REDUCERE
                                    </span>
                                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 p-6 mb-8 space-y-8">
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`relative py-4 px-4 rounded-xl border-2 font-bold transition-all flex flex-col items-center justify-center gap-1 ${selectedDimension === dim
                                                    ...
                                                    : "border-gray-100 bg-white text-gray-600 dark:text-gray-400 hover:border-gray-200 dark:border-slate-800"}`}
                                            >
                            ...
                            <div className="space-y-4 pt-4">
                                <button
                                    ...
                                    className="w-full py-4 text-lg font-bold bg-emerald-600 text-white rounded-xl shadow-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 active:scale-95"
                                >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-900 dark:text-white">Livrare Rapidă</h4>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-900 dark:text-white">Calitate Premium</h4>
                ...
                <div className="mt-8 lg:mt-16 bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 overflow-hidden">
                    ...
                    <div className="p-6 lg:p-8">
                        {activeProductTab === 'descriere' && (
                            <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300">
                                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 uppercase tracking-wider">Detalii Produs</h3>
                                <div className="whitespace-pre-line leading-relaxed mb-8">
                                    {/* Custom Description Renderer that filters out specific phrases if needed */}
                                    {(product.description || product.descriptionOriginal || "Nu există descriere pentru acest produs.")
                                        .replace(/Artist:.*\n?/, '') // Remove "Artist: Name"
                                        .replace(/.*Sabrina Seck.*\n?/, '') // Remove specific artist name if present in text
                                        .replace(/.*30 de zile.*\n?/, '') // Remove 30 days return
                                        .replace(/Titlu Oficial:.*\n?/, '') // Remove title repetition
                                        .replace(/Anul Creației:.*\n?/, '') // Remove year
                                        .replace(/Stil:.*\n?/, '') // Remove style
                                    }
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-gray-100 pt-8">
                                    <div>
                                        <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Materiale & Calitate</h4>
                                    ...
                                    <div>
                                        <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Servicii Incluse</h4>
                                        <ul className="space-y-3">
                                            <li className="flex gap-2">
                                                <Check size={18} className="text-emerald-600 shrink-0 mt-0.5" />
            ...
            {!renderOnlyConfigurator && (
                <div className="container mx-auto px-4 mt-16 pb-20">
                    <ProductCarousel title="Design-uri Populare" currentSlug={productSlug} />
                </div>
            )}
        </div>
        </div>
