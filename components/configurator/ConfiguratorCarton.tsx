...
const productFaqs: QA[] = [
    { question: "Care este diferența dintre ondulat și reciclat?", answer: "Cartonul ondulat (CO) este cel clasic, format din straturi de hârtie onduită, ideal pentru cutii. Cartonul reciclat (Honeycomb) are o structură de fagure, fiind mult mai rigid și ușor, ideal pentru display-uri și mobilier eco." },
    { question: "Ce grosimi sunt disponibile?", answer: "Ondulat: micro-ondula (1-1.5mm), CO3 (3mm) și CO5 (5mm). Reciclat: 10mm și 16mm." },
    { question: "Rezistă la exterior?", answer: "Cartonul este o soluție de interior. Expunerea la umiditate sau ploaie va degrada materialul rapid." },
];
...
export default function ConfiguratorCarton({ initialWidth: initW, initialHeight: initH, productImage }: { initialWidth?: number; initialHeight?: number; productImage?: string }) {
    ...
    function handleAddToCart() {
        ...
        const uniqueId = `carton-${Date.now()}`;
        const matLabel = input.material === "ondulat" ? `Carton Ondulat ${input.ondula}` : `Carton Fagure ${input.reciclatBoard}`;
        const title = `${matLabel} - ${input.width_cm}x${input.height_cm} cm`;
        ...
        addItem({
            ...
            metadata: {
                ...
                "Print": input.printDouble ? "Față-Verso" : "O singură față",
    ...
    const summaryStep2 = input.material === "ondulat" ? `Ondulat (${input.ondula})` : `Fagure (${input.reciclatBoard})`;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div className="lg:sticky top-24 h-max space-y-6">
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 overflow-hidden">
                            <div className="aspect-square relative flex items-center justify-center p-4">
                                <img src={activeImage} alt="Carton" className="max-h-full max-w-full object-contain" />
                            </div>
                    ...
                    <div>
                        <header className="mb-6">
                            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white mb-2">Configurator Carton</h2>
                            <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">Plăci din carton ondulat sau fagure, personalizate prin imprimare directă UV.</p>
                        </header>
                        ...
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 px-4 mb-8">
                            ...
                            <AccordionStep stepNumber={2} title="Material & Tip" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Tip Carton</label>
                                        <div className="grid grid-cols-2 gap-2">
                                            <OptionButton active={input.material === "ondulat"} onClick={() => updateInput("material", "ondulat")} title="Ondulat" subtitle="Hârtie" />
                                            <OptionButton active={input.material === "reciclat"} onClick={() => updateInput("material", "reciclat")} title="Fagure" subtitle="Masiv" />
                                        </div>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Specificație</label>
                                        {input.material === 'ondulat' ? (
                                            <select className="w-full p-3 border border-gray-300 rounded-lg bg-white" value={input.ondula} onChange={(e) => updateInput("ondula", e.target.value)}>
                                                <option value="E">Micro-ondula E (1.5mm)</option>
                                                <option value="3B">Ondula B (3mm)</option>
                                                <option value="5BC">Ondula BC (5mm)</option>
                                            </select>
                                        ) : (
                                            <select className="w-full p-3 border border-gray-300 rounded-lg bg-white" value={input.reciclatBoard} onChange={(e) => updateInput("reciclatBoard", e.target.value)}>
                                                <option value="board10">Honeycomb 10mm</option>
                                                <option value="board16">Honeycomb 16mm</option>
                                            </select>
                                        )}
                                    </div>
                                </div>
                                <label className="flex items-center gap-3 py-2 cursor-pointer">
                                    <input type="checkbox" className="w-5 h-5 rounded border-gray-300 text-slate-950 focus:ring-emerald-500" checked={input.printDouble} onChange={(e) => updateInput("printDouble", e.target.checked)} />
                                    <span className="text-sm font-bold text-gray-700 dark:text-gray-300">Imprimare Față-Verso</span>
                                </label>
                            </AccordionStep>
                                ...
                                <div>
                                    {/* Editor Online button removed from here, moved to tabs below */}
                                        ...
                                        <div className="flex -mb-px overflow-x-auto no-scrollbar">
                                            ...
                                            <Link 
                                                href={`/editor?w=${input.width_cm}&h=${input.height_cm}&product=carton`}
                                    ...
                                    {input.designOption === 'pro' && (
                                        <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">
                                            ...
                                            <p>Cost: <strong>{formatMoneyDisplay(CARTON_CONSTANTS.PRO_DESIGN_FEE)}</strong>. Un designer va realiza layout-ul dorit.</p>
                                        </div>
                                    )}
                                </div>
                            </AccordionStep>
                    ...
                    <div className="p-6 lg:p-8">
                        {activeProductTab === 'descriere' && (
                            <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 text-sm sm:text-base">
                                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Carton Personalizat - Soluția Eco de Signalistică</h2>
                                <p className="leading-relaxed mb-6">
                                    Cartonul este materialul preferat pentru soluții publicitare temporare sau sustenabile. De la clasicul carton ondulat pentru ambalaje, până la cartonul structural (fagure) pentru mobilier și standuri, oferim imprimare UV de înaltă rezoluție direct pe material.
                                </p>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Tipuri de Material</h3>
                                        <ul className="space-y-2 list-disc pl-5">
                                            <li><strong>Carton Ondulat:</strong> Disponibil în variate grosimi (E, B, BC), ideal pentru plăci de informare sau display-uri ușoare.</li>
                                            <li><strong>Carton Fagure (Honeycomb):</strong> Miez cu structură de hexagon care oferă o rigiditate uimitoare la o greutate infimă.</li>
                                            <li><strong>Eco-Friendly:</strong> 100% reciclabil, material obținut din surse controlate.</li>
                                        </ul>
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Aplicații</h3>
                                        <p>Standuri expoziționale ușoare, siluete decupate (totemuri umane), plăcuțe de preț, ambalaje prototip sau decorațiuni de interior pentru evenimente.</p>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {detailsOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setDetailsOpen(false)}>
                    <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-8">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Informații Pregătire Fișiere</h3>
                            <div className="space-y-6 text-gray-700 dark:text-gray-300 text-sm sm:text-base">
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">Print UV direct</h4>
                                    <p>Imprimăm direct pe fața albă a cartonului. Vă rugăm să trimiteți fișierele în format CMYK pentru a evita diferențele de culoare.</p>
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">Debitarea</h4>
                                    <p>Cartonul ondulat se debitează cu ușurință pe orice formă vectorială. Dacă aveți nevoie de forme complexe, alegeți opțiunea Design Pro pentru a vă asista un specialist.</p>
                                </div>
                            </div>
                        </div>
                    </div>
