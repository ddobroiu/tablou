...
export default function ConfiguratorAlucobond({ initialWidth: initW, initialHeight: initH, productImage }: { initialWidth?: number; initialHeight?: number; productImage?: string }) {
    ...
    function handleAddToCart() {
        if (!input.width_cm || !input.height_cm) {
            alert("Introduceți dimensiunile panoului."); return;
        }
        if (displayedTotal <= 0) {
        ...
        const uniqueId = `alucobond-${Date.now()}`;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                    ...
                    <div className="lg:sticky top-24 h-max space-y-6">
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 overflow-hidden">
                            <div className="aspect-square relative flex items-center justify-center p-4">
                                <img src={activeImage} alt="Alucobond" className="max-h-full max-w-full object-contain" />
                            </div>
                    ...
                    <div>
                        <header className="mb-6">
                            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white mb-2">Configurator Alucobond</h2>
                            <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">Panouri compozite din aluminiu pentru reclame durabile și fațade premium.</p>
                        </header>
                        ...
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.05)] border border-gray-200 dark:border-slate-800 px-4 mb-8">
                            ...
                            <AccordionStep stepNumber={2} title="Specificații Material" summary={summaryStep2} isOpen={activeStep === 2} onClick={() => setActiveStep(2)}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Culoare Placă</label>
                                        <select className="w-full p-3 border border-gray-300 rounded-lg bg-white" value={input.color} onChange={(e) => updateInput("color", e.target.value)}>
                                ...
                                <div>
                                    {/* Editor Online button removed from here, moved to tabs below */}
                                        ...
                                        <div className="flex -mb-px overflow-x-auto no-scrollbar">
                                            ...
                                            <Link 
                                                href={`/editor?w=${input.width_cm}&h=${input.height_cm}&product=alucobond`}
                                    ...
                                    {input.designOption === 'upload' && (
                                        <div className="space-y-3">
                                            <label className="flex flex-col items-center justify-center w-full h-32 px-4 bg-white border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-emerald-400 transition-colors">
                                                ...
                                                <span className="font-medium text-gray-600 dark:text-gray-400">Încarcă Proiect (PDF/AI)</span>
                                            ...
                                            {artworkUrl && <p className="text-sm text-green-600 font-semibold mt-2">Fișier recepționat!</p>}
                                        </div>
                                    )}
                                    ...
                                    {input.designOption === 'pro' && (
                                        <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">
                                            ...
                                            <p>Cost: <strong>{formatMoneyDisplay(ALUCOBOND_CONSTANTS.PRO_DESIGN_FEE)}</strong>. Un designer va realiza layout-ul panoului tău.</p>
                                        </div>
                                    )}
                                </div>
                            </AccordionStep>
                    ...
                    <div className="p-6 lg:p-8">
                        {activeProductTab === 'descriere' && (
                            <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 text-sm sm:text-base">
                                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Alucobond (Bond) - Rigiditate și Eleganță</h2>
                                <p className="leading-relaxed mb-6">
                                    Alucobond este soluția supremă pentru signalistica outdoor și placări de fațade. Datorită celor două straturi de aluminiu și miezului de polietilenă, panoul rămâne perfect plan chiar și la dimensiuni foarte mari (până la 3-4 metri lungime per foaie).
                                </p>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Avantaje Alucobond</h3>
                                        <ul className="space-y-2 list-disc pl-5">
                                            <li><strong>Planeitate Perfectă:</strong> Nu se ondulează sub acțiunea razelor solare.</li>
                                            <li><strong>Finisaj de Lux:</strong> Disponibil în culori standard (Alb, Gri Silver, Negru) sau texturi (Lemn, Piatră).</li>
                                            <li><strong>Rezistență la Foc:</strong> Materialul este clasificat cu o rezistență bună la incendii.</li>
                                        </ul>
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">Utilizări</h3>
                                        <p>Reclame deasupra intrării în magazine, totemuri publicitare, plăci de firmă profesionale, decorări de fațade rezidențiale sau comerciale.</p>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {detailsOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setDetailsOpen(false)}>
                    <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-8">
                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Specificații Alucobond</h3>
                            <div className="space-y-6 text-gray-700 dark:text-gray-300 text-sm sm:text-base">
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">Grosime și Greutate</h4>
                                    <p>Placa standard de 3mm are o greutate de aprox. 4kg/mp, fiind mult mai ușoară decât o placă solidă de oțel sau aluminiu la aceeași rigiditate.</p>
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white mb-2">Montaj și Prelucrare</h4>
                                    <p>Alucobond-ul se prelucrează exclusiv mecanic (frezare/CNC). Se poate caseta dacă se frezează pe spate un canal în V, permițând îndoirea precisă la 90 de grade.</p>
                                </div>
                            </div>
                        </div>
                    </div>
