...
for (const [id, data] of Object.entries(products)) {
    ...
    if (match) {
        const afterIdIndex = match.index + match[0].length;
        const titleRegex = /"title":\s*"(.*?)"/;
        const searchString = content.slice(afterIdIndex);
        const titleMatch = titleRegex.exec(searchString);

        if (titleMatch) {
            // Check if another ID appeared before this title
            const nextId = /"id":/.exec(searchString.slice(0, titleMatch.index));
            if (!nextId) {
                const absoluteTitleStart = afterIdIndex + titleMatch.index;
...
for (const [id, data] of Object.entries(products)) {
    ...
    if (match) {
        ...
        if (descMatch) {
            ...
            if (!nextId) {
                const absoluteDescStart = afterIdIndex + descMatch.index;
